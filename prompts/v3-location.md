...
- **Region / state / county** → `"Region, Country"` — append the country for the same disambiguation reason (Bavaria-Germany, California-United States, Île-de-France-France).
  - "developers in Provence" → `"Provence, France"`
...
- EXCEPTION: "remote-friendly but prefer Paris" → DO set LOCATION = `"Paris, France"` with isRequired: false (it's a soft preference).
...
- "developers in Provence" → LOCATION = `"Provence, France"` (region + country)
