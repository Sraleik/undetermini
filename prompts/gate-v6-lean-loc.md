...
- "X" alone usually = country ("Mexico", "Luxembourg", "Georgia"); the "City" variant = city ("Mexico City, Mexico"). City-states ("Singapore"/"Monaco") → name once, no duplication.
- US-state traps: "Washington" alone → "Washington, D.C., United States"; "Washington State" → "Washington, United States". "Georgia" → country unless US context ("Atlanta, Georgia" → "Georgia, United States").
...
LOCATION examples (City, Country format throughout; radius/exclusion as marked):
- "dev React à Paris" → "Paris, France", isRequired: true, radius: null
- "engineers in the Bay Area" → "San Francisco, United States", radius: 50
- "developers in Provence" → "Provence-Alpes-Côte d'Azur, France" (region → official admin région)
- "Devs based in Brooklyn" → "New York, United States" (neighborhood → parent city)
- "Paris, Texas" → "Paris, United States" (state implies country; only country in valueText)
- "marketers in France or Germany" → 2 LOCATIONs ("France", "Germany"), both isRequired: false
- "dev senior pas à Lyon" → "Lyon, France", isExcluded: true

Translation rules:
- The user may write in any language (French, Spanish, etc.), but LOCATION and LANGUAGE filter values MUST always be in English.
- For LOCATION: translate city/country/region names to their canonical English form, THEN apply VALUE FORMAT (City, Country) — e.g. "candidats à Londres" → "London, United Kingdom".
