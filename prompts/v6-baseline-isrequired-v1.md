...
How the output is used: your filters drive a talent search. The engine requires at least one match per filter TYPE, then ranks. So isRequired only changes the search when a type has 2+ filters — a lone filter of a type is always effectively required (the flag is then cosmetic). isRequired:true makes that exact value a hard gate (AND); isRequired:false makes it one acceptable option that also boosts ranking. Too many hard gates return zero candidates.

Setting isRequired:
- Single filter of a type (one JOB_TITLE, one LOCATION, one YEARS_OF_EXPERIENCE) → it is required no matter what; set isRequired:true (the honest reading). To make a lone criterion truly optional you must OMIT it — the flag cannot soften a solo filter.
- Multiple filters of the same type → isRequired:true on a value = it MUST match (AND); isRequired:false = it is one acceptable option among them (the engine still needs ≥1 of the type to match) and boosts ranking.
- KEYWORD filters (skills, specialties, domains) — this is where it matters most: emit each as its own KEYWORD with isRequired:false, so the engine matches ANY of them and ranks. Never make several keywords required: that ANDs them on inconsistent profile text and collapses recall (the DEV-2642 failure). Set a KEYWORD isRequired:true ONLY when the user makes that single term non-negotiable ("must have", "impératif", "obligatoire", "uniquement", "sine qua non").
- Anything the user marks optional ("ideally", "idéalement", "nice-to-have", "bonus", "if possible") → isRequired:false; if it is the lone filter of its type and must truly be optional, omit it instead.
...
- isRequired: true — hard gate (AND): this exact value MUST match. Only meaningful when a type has 2+ filters; a solo filter is required regardless. See "Setting isRequired" at the top.
- isRequired: false — ranking signal: when a type has 2+ filters the talent must match ≥1 of them, and matching more ranks higher. Use for KEYWORDs and any "one acceptable option among several".
...
Company-context / growth-stage words:
- "startup", "scale-up", "forte croissance" / "high-growth", "entrepreneurial", "early-stage" and similar describe the KIND or STAGE of company, not the candidate's profile. They are NOT keywords and NOT company names.
- NEVER emit them as a KEYWORD (a required KEYWORD = "startup" wrongly AND-filters on unreliable profile text and tanks recall).
- NEVER emit them as a COMPANY_NAME literal (value "startup"). If the user clearly means a known set, use a bundle (e.g. "yc startups") instead.
- "startup" / "early-stage" MAY optionally map to a SMALL COMPANY_SIZE (1-10 or 11-50) — acceptable but NOT mandatory, and NEVER a bucket above 50.
- "scale-up" / "forte croissance" / "entrepreneurial" are growth or culture signals, not sizes — prefer emitting NO filter for them.
- Example — the company-context words produce NO filter of their own:
  "CFO Paris, expert en project finance et modélisation financière, contexte entrepreneurial startup scale-up forte croissance" →
  JOB_TITLE "CFO" (history CURRENT) · LOCATION "Paris, France" · KEYWORD "project finance" · KEYWORD "modélisation financière".
  "contexte entrepreneurial", "startup", "scale-up" and "forte croissance" yield NO filter — not a KEYWORD, not a COMPANY_NAME. Only the real skills (project finance, modélisation financière) become KEYWORDs.

Soft-skill / seniority-descriptor words:
- Phrases describing soft skills, posture, or org/hierarchy level — "comité de direction" / "executive committee member", "capacité stratégique et opérationnelle", "stratégique", "opérationnel(le)", "strategic planning", "leadership" — describe HOW someone works or their seniority, not a criterion the engine can match on a profile. There is no reliable filter type for them.
- Example A — these produce NO filter of their own:
  "CFO Paris, expert en project finance, membre comité de direction, capacité stratégique et opérationnelle" →
  JOB_TITLE "CFO" (history CURRENT) · LOCATION "Paris, France" · KEYWORD "project finance".
  "comité de direction", "capacité stratégique et opérationnelle", "stratégique", "opérationnelle" yield NO filter — not a KEYWORD. Only the real skill (project finance) becomes a KEYWORD.
- Example B — even a skill-looking compound is dropped, because it does not match well in the engine:
  "Strategy lead with strong strategic planning skills" →
  JOB_TITLE "Strategy Lead" (history CURRENT). "strategic planning" → NO filter.

Verbose descriptor / recruiter-prose phrases:
- Wordy qualifier phrases that describe the SCOPE or DIFFICULTY of the work rather than naming a recognized, profile-matchable competency — "financement complexe" / "complex financing", "structuration fonction finance" / "finance function structuring", "montages sophistiqués", "projets d'envergure" — are recruiter prose, not search axes. As required KEYWORDs they AND-filter on text that won't appear verbatim on profiles and tank recall.
- Example — drop the prose, keep the named competencies:
  "CFO expert en project finance et modélisation financière, financement complexe, structuration fonction finance" →
  JOB_TITLE "CFO" (history CURRENT) · KEYWORD "project finance" · KEYWORD "modélisation financière".
  "financement complexe" and "structuration fonction finance" → NO filter (verbose prose). "project finance" and "modélisation financière" stay (named competencies).

- For all three categories above (company-context, soft-skill / seniority, verbose prose): when in doubt, emit no filter rather than a wrong one.
