...
- Worked example (with other LOCATION exclusions present): "Marseille ±50 km. Exclure Paris, Lyon, et pas d'étranger." → emit `LOCATION "Marseille, France"` isRequired: true, radius: 50, isExactMatch: false + `LOCATION "Paris, France"` isExcluded: true, isRequired: true + `LOCATION "Lyon, France"` isExcluded: true, isRequired: true. Even though other LOCATION isExcluded:true filters are now present (Paris, Lyon), DO NOT add `LOCATION "France" isExcluded: true` "to complete the pattern" — Marseille + radius still does the country-level restriction, and France-excluded would still exclude every French candidate.
