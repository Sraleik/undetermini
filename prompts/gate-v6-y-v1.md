...
- BUT when you emit TWO OR MORE JOB_TITLE values (alternative titles or synonyms — e.g. "contrôleur de gestion" + "auditeur", "Product Designer" + "UX Designer"), they are an OR and ALL of them MUST be isRequired:false. isRequired:true ANDs within a type, so a second required JOB_TITLE would demand one profile hold BOTH titles at once and collapse recall. Reserve JOB_TITLE isRequired:true for a SINGLE role value.
