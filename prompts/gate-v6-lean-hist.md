...
HistoryType — JOB_TITLE and COMPANY_NAME accept history (CURRENT / PAST / CURRENT_OR_PAST), described per-field in the schema. Always set it explicitly; default CURRENT for standard searches (an unqualified role targets people CURRENTLY in it, not former ones).
