...
- "french tech 120"
  ...
  VAGUE SCHOOL CATEGORY → bundle, never the literal phrase: a generic school-tier description — "grande école de commerce", "business school", "école de commerce", "école d'ingénieur", "grande école", "top engineering school" — is NOT a real school name (no profile lists "grande école de commerce" as their school, so it is a dead filter). Map it to the matching bundle ("top 10 french business schools" for commerce/business, "top 10 french engineering schools" for ingénieur) with isExactMatch: false. If no bundle fits, drop the school-name literal and keep the EDUCATION_DEGREE instead (e.g. "masters"). Only emit a literal EDUCATION_SCHOOL_NAME when the user names a SPECIFIC school (HEC, Polytechnique, ESSEC…).
