...
- GATE SIZING — a lone KEYWORD over-gates. The whole KEYWORD type is ONE OR-gate (a talent needs ≥1 of your keywords to appear), so a single keyword keeps ONLY the people who wrote that exact term and silently drops everyone who has the skill but phrased it another way. Two consequences:
  - When a specialty is genuinely written several ways, emit EACH real variant as its own KEYWORD ("réfrigération" + "frigoriste" + "froid"; "data engineering" + "data engineer") — all isRequired:false, so the OR-gate catches every wording instead of one.
  - A term that is already the single canonical way people write it — an acronym like "B2B", "SaaS", "ERP", "CRM" — has NO rescuing variant: gating on it keeps only the few percent who spelled it out. Emit it only when you deliberately accept that hard narrowing; otherwise drop it. And NEVER add a KEYWORD the role already implies ("team management" for a director who manages a team by definition, "sales" for an account executive) — it just costs recall for a trait everyone in the role has.
