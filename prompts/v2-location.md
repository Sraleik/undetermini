...
GRANULARITY — choose the right value for the LOCATION:
- City: use the city name when the user names one ("Lyon", "Berlin"). Most common case.
- Country: use the country name when the user only specifies a country ("based in France", "anywhere in Germany").
- Region / state / county: use the region name verbatim ("Provence", "Bavaria", "California", "Île-de-France"). The search engine resolves it.
- Neighborhood / district: substitute with the parent city. "Brooklyn" → "New York", "Le Marais" → "Paris", "Shibuya" → "Tokyo", "Kreuzberg" → "Berlin", "Soho" → "London", "Mission District" → "San Francisco".

CITY + COUNTRY DISAMBIGUATION — when the user gives both, prefer the city (more specific):
- "Paris, France" → LOCATION = "Paris"
- EXCEPTION: country is required to disambiguate same-named cities. "Paris, Texas" → LOCATION = "Paris, Texas". "Cambridge, UK" → "Cambridge, United Kingdom" (vs "Cambridge, Massachusetts").

AMBIGUOUS PLACE NAMES — resolve by context:
- "Mexico" alone → country. "Mexico City" → city.
- "Luxembourg" alone → country. "Luxembourg City" / "in downtown Luxembourg" → city.
- "Singapore" / "Monaco" / "Vatican" alone → treat as city (city-states).
- "Washington" alone → "Washington, D.C." (the capital). "Washington State" → "Washington".
- "Georgia" alone → country (US state is less common in talent search; if user mentions an American context like "Atlanta, Georgia", treat as state).

ABBREVIATIONS — always expand to the full canonical English name:
- "USA" / "US" / "U.S." → "United States"
- "UK" / "U.K." → "United Kingdom"
- "UAE" → "United Arab Emirates"
- "NYC" / "NY" → "New York"
- "LA" → "Los Angeles" (NOT "Louisiana"; default to the city)
- "SF" → "San Francisco"
- "DC" → "Washington, D.C."
- "BCN" → "Barcelona"
- "PHL" / "Philly" → "Philadelphia"

METROPOLITAN AREA / GREATER REGION — when the user signals a wider area, set a radius and isExactMatch: false:
...
- Wider known metros: "Bay Area" / "Silicon Valley" → "San Francisco" + radius=50; "Greater London" → "London" + radius=30; "Greater Paris" / "Paris area" / "Île-de-France" as metro → "Paris" + radius=30; "Ruhr" → "Essen" + radius=50.

EXPLICIT DISTANCE — overrides the metro-area default:
- "within 10 km of Paris" → "Paris" + radius=10, isExactMatch: false
- "10 miles from Boston" → "Boston" + radius=16 (1 mile ≈ 1.6 km, round to nearest km)
- "30 minutes from Lyon" → "Lyon" + radius=25 (interpret commute time as urban ~50 km/h)
- Always set isExactMatch: false when radius is non-null.

REMOTE WORK / "ANYWHERE" — do NOT set any LOCATION filter:
- Triggers: "remote", "fully remote", "100% remote", "WFH", "work from home", "anywhere", "any location", "no location preference", "distributed team", "telework"
- Even when the user adds vague geographic context ("remote in EU", "remote, French speakers", "remote anywhere in Europe"), still skip LOCATION — those are language/timezone hints, not LOCATION filters.
- EXCEPTION: "remote-friendly but prefer Paris" → DO set LOCATION = "Paris" with isRequired: false (it's a soft preference).
...
CASING — always Title Case the value: "London", "New York", "São Paulo", "Île-de-France", "Côte d'Ivoire". Even if the user typed lowercase.

LOCATION examples:
- "dev React à Paris" → LOCATION = "Paris", isRequired: true, isExactMatch: true, radius: null
- "engineers in the Bay Area" → LOCATION = "San Francisco", radius: 50, isExactMatch: false, isRequired: true
- "marketers based in France or Germany" → 2 LOCATIONs ("France", "Germany"), both isRequired: false
- "designer remote en Europe" → no LOCATION filter (remote)
- "dev senior pas à Lyon" → LOCATION = "Lyon", isExcluded: true, isRequired: true
- "candidates near Berlin" → LOCATION = "Berlin", radius: 30, isExactMatch: false
- "consultants à Mexico" → LOCATION = "Mexico" (country, default when ambiguous)
- "Devs at YC startups based in Brooklyn" → LOCATION = "New York" (neighborhood → parent city)
- "developers in Provence" → LOCATION = "Provence" (region kept verbatim)
- "in the US" → LOCATION = "United States" (expand abbreviation)
- "Paris area" → LOCATION = "Paris", radius: 30, isExactMatch: false
- "candidats à Paris, France" → LOCATION = "Paris" (city wins over country when both given)
- "Paris, Texas" → LOCATION = "Paris, Texas" (disambiguation kept)
- "within 20 km of Bordeaux" → LOCATION = "Bordeaux", radius: 20, isExactMatch: false
- "10 miles from Boston" → LOCATION = "Boston", radius: 16
- "candidats à Londres, idéalement à Manchester" → 2 LOCATIONs ("London", "Manchester"), London isRequired: true, Manchester isRequired: false (ideally = soft preference)

Translation rules:
- The user may write in any language (French, Spanish, etc.), but LOCATION and LANGUAGE filter values MUST always be in English.
- For LOCATION: translate city, country, and region names to their canonical English form. Common translations to apply:
  "Londres" → "London"
  "Allemagne" → "Germany"
