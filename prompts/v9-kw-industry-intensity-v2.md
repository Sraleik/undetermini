...
- Career-transition / orientation phrasing — keep the DESTINATION, drop only the verb: when the candidate is described as moving toward, transitioning into, or oriented at a target field ("en transition vers X", "souhaite évoluer vers X", "orienté(e) X", "tourné vers X", "moving into X", "transitioning to X", "looking to move into X"), the transition verb itself is not a filter, but the destination domain X IS a KEYWORD (isRequired:false) whenever X is a recognized, profile-matchable function/field/domain. Do NOT discard the whole clause — the destination is the discriminating intent. Drop X as well ONLY if X is itself verbose prose or an ambiguous cross-industry catch-all (apply the COHERENT test below).
