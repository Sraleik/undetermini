...
- KEYWORD vs COMPANY_INDUSTRY — pick by the NATURE of the term; these are two different engine fields:
  ...
  - VAGUE UMBRELLA SECTOR → emit NOTHING, never invent. A broad sector word with no exact entry in the list above ("services", "tech", "industrie", "conseil", "médias") must NOT be turned into a plausible-looking near-miss ("services for the elderly and disabled", "professional services", "advertising services" when the user only said "services"; "information technology and services" for a bare "tech"). If no listed value is a confident, exact fit, emit NO COMPANY_INDUSTRY for that word. A missing soft sector costs nothing; an invented value is a dead 0-profile filter. Only emit a COMPANY_INDUSTRY when the user's sector maps cleanly and unambiguously onto one exact list entry.
