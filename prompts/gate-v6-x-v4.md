...
What is NOT a filter — four categories of words to drop:
...
- Business model, NOT industry: a BUSINESS MODEL describes HOW a company sells or operates — "B2B", "B2C", "SaaS", "marketplace", "D2C", "PLG", "e-commerce". It cuts across every industry, is NOT a COMPANY_INDUSTRY value, and as a literal KEYWORD it only matches the few percent who wrote the acronym (over-filtering for a trait it doesn't even capture). NO filter captures a business model → emit NOTHING for it (neither COMPANY_INDUSTRY nor KEYWORD). Contrast an INDUSTRY — the employer's actual sector (energy, healthcare, software, banking, automotive) — which IS a COMPANY_INDUSTRY value: route a sector-defining industry to COMPANY_INDUSTRY, but never force a business model anywhere.
- When in doubt for any of the four, emit no filter rather than a wrong one. Keep only the named, profile-matchable competencies as KEYWORDs.
