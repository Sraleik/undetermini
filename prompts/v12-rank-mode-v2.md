...
How filters are used: isRequired:true makes a filter a hard gate — talents not matching it are excluded. isRequired:false makes it a ranking signal — talents matching it rank higher, but non-matching talents are still returned. Multiple isRequired:false filters of the same type let the engine rank by how many match; multiple isRequired:true on the same type AND-collapse and crush recall. A lone isRequired:false filter is genuinely optional (it ranks, it does not gate). Too many hard-required filters across types return zero candidates.
...
- Multiple filters of the SAME type offered as alternatives ("Product Designer or UX Designer", "Paris, Lyon or Bordeaux") → isRequired:false on each: the engine ranks talents matching more of them higher. (Excluded LOCATIONs are the exception — see LOCATION rules.)
- Optional markers ("ideally", "idéalement", "nice-to-have", "bonus", "if possible") → isRequired:false; a lone isRequired:false filter is genuinely optional (it ranks but does not gate).
...
- COMPANY_INDUSTRY — the EMPLOYER's sector. Pick the closest allowed value (use the classic LinkedIn industry form, lowercase); if none fits, emit no COMPANY_INDUSTRY rather than an invented or near-miss value. For a BROAD/UMBRELLA sector word ("services", "tech", "industry"), pick the most ENCOMPASSING applicable allowed value, not a narrow niche — choose a specific niche (e.g. "hospitality", "consumer services") only when the JD clearly points to it
...
- isRequired — see "Setting isRequired" above. true = the talent MUST match this value; false = ranking signal only (talents matching it rank higher, non-matching still returned). A lone isRequired:false filter is genuinely optional — it ranks but does not gate.
