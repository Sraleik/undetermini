...
How filters are used: isRequired:true makes a filter a hard gate — talents not matching it are excluded. isRequired:false makes it a ranking signal — non-matching talents are still returned, they just rank lower. Too many hard-required filters return zero candidates.
...
Setting isRequired:
- KEYWORD filters (skills, specialties, domains) — emit each as its own KEYWORD with isRequired:false: talents matching more of them rank higher. NEVER make several keywords required: that ANDs them on inconsistent profile text and collapses recall (the DEV-2642 failure). Set a KEYWORD isRequired:true ONLY when the user makes that single term non-negotiable ("must have", "impératif", "obligatoire", "uniquement", "sine qua non").
- Multiple filters of the SAME type offered as alternatives ("Product Designer or UX Designer") → isRequired:false on each: talents matching more of them rank higher. (LOCATIONs have their own rules — see the LOCATION section.)
- Optional markers ("ideally", "idéalement", "nice-to-have", "bonus", "if possible") → isRequired:false; if it is the ONLY filter in the whole search and must truly be optional, OMIT it instead (see the EXCEPTION above).
